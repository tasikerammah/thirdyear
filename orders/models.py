...
class OrderManagerQuerySet(models.query.QuerySet):
    ...
    def get_sales_breakdown(self):
        ...
        derivared = recent.not_refunded().by_status(status='derivared')
        derivared_data = derivared.totals_data()
    ...
    def by_status(self, status="derivared"):
...
class Order(models.Model):
    ...
    residence           = models.ForeignKey(Address, related_name="shipping_address",null=True, blank=True,on_delete=models.CASCADE)
    personal_pick       = models.ForeignKey(Address, related_name="billing_address", null=True, blank=True,
                                            on_delete=models.CASCADE)
    cart                = models.ForeignKey(Cart,on_delete=models.CASCADE)
    status              = models.CharField(max_length=120, default='created', choices=ORDER_STATUS_CHOICES)
    residence_total     = models.DecimalField(default=5.99, max_digits=50,decimal_places=2)
    ...
    def get_status(self):
        ...
        elif self.status == "derivared":
            return "Derivared"
        return "Derivaring Soon"

    def update_total(self):
        cart_total = self.cart.total
        residence_total = self.residence_total
        new_total = math.fsum([cart_total, residence_total])
    ...
    def check_done(self):
        residence_required = not self.cart.is_digital
        residence_done = False
        if residence_required and self.residence:
            derivaring_done = True
        elif residence_required and not self.residence:
            derivaring_done = False
        else:
            derivaring_done = True
        billing_profile = self.billing_profile
        total   = self.total
        if billing_profile and derivaring_done and total > 0:
            return True
        return False
...
def pre_save_create_order_id(sender, instance, *args, **kwargs):
    ...
    if instance.residence and not instance.shipping_address_final:
